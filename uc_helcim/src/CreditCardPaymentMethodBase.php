...
/**
 * Defines a Helcim credit card payment method plugin implementation.
 */
abstract class CreditCardPaymentMethodBase extends PaymentMethodPluginBase {
   ...
   *
   * @return array
   *   An array with values UC_HELCIM_AUTH_ONLY, UC_HELCIM_PRIOR_AUTH_CAPTURE,
   *   UC_HELCIM_AUTH_CAPTURE, UC_HELCIM_REFERENCE_SET, UC_HELCIM_REFERENCE_TXN,
   *   UC_HELCIM_REFERENCE_REMOVE, UC_HELCIM_REFERENCE_CREDIT, UC_HELCIM_CREDIT
   *   and UC_HELCIM_VOID.
   */
  public function getTransactionTypes() {
    return [
      UC_HELCIM_AUTH_CAPTURE,
      UC_HELCIM_AUTH_ONLY,
    ];
  }
  ...
  public function getDisplayLabel($label) {
    $build['#attached']['library'][] = 'uc_helcim/uc_helcim.styles';
    $build['label'] = array(
      '#plain_text' => $label,
    );
    ...
    foreach ($cc_types as $type => $description) {
      $build['image'][$type] = array(
        '#theme' => 'image',
        '#uri' => drupal_get_path('module', 'uc_helcim') . '/images/' . $type . '.gif',
        '#alt' => $description,
        '#attributes' => array('class' => array('uc-helcim-cctype', 'uc-helcim-cctype-' . $type)),
      );
    }
    return $build;
  }
  ...
  public function defaultConfiguration() {
    return [
      'txn_type' => UC_HELCIM_AUTH_CAPTURE,
    ];
  }
  ...
  public function buildConfigurationForm(array $form, FormStateInterface $form_state) {
    $form['txn_type'] = [
      ...
      '#options' => [
        UC_HELCIM_AUTH_CAPTURE => $this->t('Authorize and capture immediately'),
        UC_HELCIM_AUTH_ONLY => $this->t('Authorization only'),
      ],
    ];
  ...
  public function cartDetails(OrderInterface $order, array $form, FormStateInterface $form_state) {
    $build = array(
      '#type' => 'container',
      '#attributes' => array('class' => 'uc-helcim-form')
    );
    $build['#attached']['library'][] = 'uc_helcim/uc_helcim.styles';
    ...
    if ($session->has('sescrd')) {
      $order->payment_details = uc_helcim_cache($session->get('sescrd'));
    ...
    if($this->configuration['payment_method'] == '#helcimJS'){
      ...
      $build['cc_number'] = array(
        ...
        '#attributes' => array('autocomplete' => 'off', 'id' => 'cc_number', 'onkeyup' => 'javascript: return hcmClearResponse();'),
        '#size' => 20,
        '#maxlength' => 19,
      );
      ...
      $build['cc_number'] = array(
        ...
        '#title' => $this->t('Card number'),
        '#default_value' => $default_num,
        '#attributes' => array('autocomplete' => 'off', 'id' => 'cc_number'),
        '#size' => 20,
        '#maxlength' => 19,
      );
    ...
    if($this->configuration['payment_method'] == '#helcimJS'){
      ...
      $build['cc_number']['#required'] = FALSE;
      $build['cc_exp_month']['#required'] = FALSE;
      $build['cc_exp_year']['#required'] = FALSE;
      $build['cc_cvv']['#required'] = FALSE;

      // TOKEN FIELD
      $build['token'] = array(
        '#type' => 'hidden',
        '#title' => $this->t('Helcim JS Token'),
        '#default_value' => $this->configuration['helcim_js_token'],
        '#attributes' => array('autocomplete' => 'off', 'id' => 'token'),
        '#size' => 24,
        '#maxlength' =>30,
      );

       // TEST MODE FIELD
       $build['dontSubmit'] = array(
        '#type' => 'hidden',
        '#default_value' => 1,
        '#attributes' => array('autocomplete' => 'off', 'id' => 'dontSubmit'),
      );

      // CARD TOKEN FIELD
      $build['card_token'] = array(
        '#type' => 'hidden',
        '#title' => $this->t('Card Token'),
        '#default_value' => '',
        '#attributes' => array('autocomplete' => 'off', 'id' => 'card_token'),
        '#size' => 24,
        '#maxlength' =>30,
      );

      // CARD F4L4 FIELD
      $build['card_f4l4'] = array(
        '#type' => 'hidden',
        '#title' => $this->t('Card F4L4'),
        '#default_value' => '',
        '#attributes' => array('autocomplete' => 'off', 'id' => 'card_f4l4'),
        '#size' => 24,
        '#maxlength' =>30,
      );
      ...
      $build['customer_code'] = array(
        '#type' => 'hidden',
        '#title' => $this->t('Customer Code'),
        '#default_value' => $user->id(),
        '#attributes' => array('autocomplete' => 'off', 'id' => 'customer_code'),
        '#size' => 24,
        '#maxlength' =>30,
      );

      // RESPONSE FIELD
      $build['helcim_response'] = array(
        '#type' => 'hidden',
        '#title' => $this->t('Response'),
        '#default_value' => '',
        '#attributes' => array('autocomplete' => 'off', 'id' => 'helcim_response'),
        '#size' => 24,
        '#maxlength' =>50,
      );

      // RESPONSE MESSAGE FIELD
      $build['helcim_response_message'] = array(
        '#type' => 'hidden',
        '#title' => $this->t('Response Message'),
        '#default_value' => '',
        '#attributes' => array('autocomplete' => 'off', 'id' => 'helcim_response_message'),
        '#size' => 24,
        '#maxlength' =>100,
      );
      ...
      $build['test'] = array(
        ...
        '#attributes' => array('autocomplete' => 'off', 'id' => 'test'),
      );
  ...
  public function orderView(OrderInterface $order) {
    ...
    if ($account->hasPermission('process credit cards')) {
      $build['terminal'] = [
        ...
        '#url' => Url::fromRoute('uc_helcim.terminal', [
          'uc_order' => $order->id(),
  ...
  public function cartProcess(OrderInterface $order, array $form, FormStateInterface $form_state) {
    ...
    if ($this->configuration['payment_method'] != '#helcimJS' and isset($cc_data['payment_details_data'])) {
      $cache = uc_helcim_cache(base64_decode($cc_data['payment_details_data']));
      unset($cc_data['payment_details_data']);
    }

    // Account for partial CC numbers when masked by the system.
    if ($this->configuration['payment_method'] != '#helcimJS' and substr($cc_data['cc_number'], 0, strlen(t('(Last4)'))) == $this->t('(Last4)')) {
    ...
    if ($this->configuration['payment_method'] != '#helcimJS' and !empty($cc_data['cc_cvv']) && $cc_data['cc_cvv'] == str_repeat('-', strlen($cc_data['cc_cvv']))) {
    ...
    if($this->configuration['payment_method'] == '#helcimJS' and $cc_data['helcim_response'] != 1){

      if(strpos($cc_data['helcim_response_message'], 'NUMBER') != FALSE){

        // INVALID CREDIT CARD NUMBER
        $form_state->setErrorByName('panes][payment][details][cc_number', $this->t($cc_data['helcim_response_message']));
        $return = FALSE;

      }
      
      if(stripos($cc_data['helcim_response_message'], 'EXPIRY') != FALSE){

        // INVALID CREDIT CARD EXPIRY
        $form_state->setErrorByName('panes][payment][details][cc_exp_month', $this->t($cc_data['helcim_response_message']));
        // INVALID CREDIT CARD EXPIRY
        $form_state->setErrorByName('panes][payment][details][cc_exp_year', $this->t($cc_data['helcim_response_message']));
        $return = FALSE;

      }
      
      if(strpos($cc_data['helcim_response_message'], 'CVV') != FALSE){

        // INVALID CREDIT CARD CVV
        $form_state->setErrorByName('panes][payment][details][cc_cvv', $this->t($cc_data['helcim_response_message']));
        $return = FALSE;
      ...
      if($return){
        ...
        $form_state->setError($form, $cc_data['helcim_response_message']);
        $return = FALSE;
    ...
    if ($this->configuration['payment_method'] != '#helcimJS' and !empty($fields['owner']) && empty($cc_data['cc_owner'])) {
      $form_state->setErrorByName('panes][payment][details][cc_owner', $this->t('Enter the owner name as it appears on the card.'));
      $return = FALSE;
    }

    // Validate the credit card number.
    if ($this->configuration['payment_method'] != '#helcimJS' and !$this->validateCardNumber($cc_data['cc_number'])) {
    ...
    if ($this->configuration['payment_method'] != '#helcimJS' and !empty($fields['start']) && !$this->validateStartDate($cc_data['cc_start_month'], $cc_data['cc_start_year'])) {
    ...
    if ($this->configuration['payment_method'] != '#helcimJS' and !$this->validateExpirationDate($cc_data['cc_exp_month'], $cc_data['cc_exp_year'])) {
    ...
    if ($this->configuration['payment_method'] != '#helcimJS' and !empty($fields['issue']) && !$this->validateIssueNumber($cc_data['cc_issue'])) {
    ...
    if ($this->configuration['payment_method'] != '#helcimJS' and !empty($fields['cvv']) && !$this->validateCvv($cc_data['cc_cvv'])) {
    ...
    if ($this->configuration['payment_method'] != '#helcimJS' and !empty($fields['bank']) && empty($cc_data['cc_bank'])) {
      $form_state->setErrorByName('panes][payment][details][cc_bank', $this->t('You must enter the issuing bank for that card.'));
      $return = FALSE;
    }

    // Initialize the encryption key and class.
    $key = uc_helcim_encryption_key();
    ...
    uc_store_encryption_errors($crypt, 'uc_helcim');
  ...
  public function orderSave(OrderInterface $order) {
    ...
    $crypt = \Drupal::service('uc_store.encryption');
    $order->data->cc_data = $crypt->encrypt(uc_helcim_encryption_key(), base64_encode(serialize($cc_data)));
    uc_store_encryption_errors($crypt, 'uc_helcim');
  }
   ...
   *   The transaction type, one of the UC_HELCIM_* constants.
