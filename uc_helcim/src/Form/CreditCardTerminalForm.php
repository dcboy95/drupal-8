...
class CreditCardTerminalForm extends FormBase {
  ...
  public function getFormId() {
    return 'uc_helcim_terminal_form';
  }
  ...
  public function buildForm(array $form, FormStateInterface $form_state, OrderInterface $uc_order = NULL, PaymentMethod $uc_payment_method = NULL) {
    ...
    if (in_array(UC_HELCIM_AUTH_CAPTURE, $types)) {
    ...
    if (in_array(UC_HELCIM_AUTH_ONLY, $types)) {
    ...
    if (in_array(UC_HELCIM_REFERENCE_SET, $types)) {
    ...
    // If available, create a reference at the gateway.
    if (in_array(UC_HELCIM_CREDIT, $types)) {
    ...
    if (!empty($options)) {
      ...
      if (in_array(UC_HELCIM_PRIOR_AUTH_CAPTURE, $types)) {
      ...
      if (in_array(UC_HELCIM_VOID, $types)) {
    ...
    if (!empty($options)) {
      ...
      if (in_array(UC_HELCIM_REFERENCE_TXN, $types)) {
      ...
      if (in_array(UC_HELCIM_REFERENCE_REMOVE, $types)) {
      ...
      // If available, remove a previously stored reference.
      if (in_array(UC_HELCIM_REFERENCE_CREDIT, $types)) {
    ...
    $form['#attached']['library'][] = 'uc_helcim/uc_helcim.styles';
