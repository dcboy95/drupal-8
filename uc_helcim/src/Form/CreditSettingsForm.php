...
namespace Drupal\uc_helcim\Form;
...
class CreditSettingsForm extends ConfigFormBase {
  ...
  public function getFormId() {
    return 'uc_helcim_settings_form';
  }
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    $config = $this->config('uc_helcim.settings');

    $form['cc_security']['uc_helcim_encryption_path'] = array(
      '#type' => 'textfield',
      ...
      '#default_value' => uc_helcim_encryption_key() ? $config->get('encryption_path') : $this->t('Not configured.'),
    );
  ...
  public function validateForm(array &$form, FormStateInterface $form_state) {
    ...
    $key_path = rtrim(trim($form_state->getValue('uc_helcim_encryption_path')), '/\\');

    // Test to see if a path was entered.
    if (empty($key_path)) {
      $form_state->setErrorByName('uc_helcim_encryption_path', $this->t('Key path must be specified in security settings tab.'));
    }

    // Construct complete key file path.
    $key_file = $key_path . '/' . UC_HELCIM_KEYFILE_NAME;
      ...
      if (is_readable($key_file)) {
        ...
        if ($key) {
          if (!preg_match("([0-9a-fA-F]{32})", $key)) {
            $form_state->setErrorByName('uc_helcim_encryption_path', $this->t('Key file already exists in directory, but it contains an invalid key.'));
          }
          else {
            // Key file exists and is valid, save result of trim() back into
            // $form_state and proceed to submit handler.
            $form_state->setValue('uc_helcim_encryption_path', $key_path);
            return;
          }
        }
      }
      else {
        $form_state->setErrorByName('uc_helcim_encryption_path', $this->t('Key file already exists in directory, but is not readable. Please verify the file permissions.'));
      }
    }
    ...
    if (is_dir($key_path)) {
      ...
      if ($file === FALSE) {
        $form_state->setErrorByName('uc_helcim_encryption_path', $this->t('Cannot write to directory, please verify the directory permissions.'));
        ...
        if (@fwrite($file, '0123456789') === FALSE) {
          ...
          if ($file === FALSE) {
            $form_state->setErrorByName('uc_helcim_encryption_path', $this->t('Cannot read from directory, please verify the directory permissions.'));
            $form_state->setValue('update_cc_encrypt_dir', FALSE);
          }
          else {
            fclose($file);
      ...
      $form_state->setErrorByName('uc_helcim_encryption_path', $this->t('You have specified a non-existent directory.'));
    }
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    if ($form_state->getValue('update_cc_encrypt_dir')) {
      $key_path = $form_state->getValue('uc_helcim_encryption_path');
      $key_file = $key_path . '/' . UC_HELCIM_KEYFILE_NAME;
      ...
      if (!file_exists($key_file)) {
        if (!$file = fopen($key_file, 'wb')) {
          ...
          $this->logger('uc_helcim')->error('Credit card encryption key file creation failed for file @file. Check your filepath settings and directory permissions.', ['@file' => $key_file]);
        }
        else {
          ...
          $this->logger('uc_helcim')->notice('Credit card encryption key file generated. Card data will now be encrypted.');
        }
      }
    }

    $this->config('uc_helcim.settings')
      ->set('encryption_path', $form_state->getValue('uc_helcim_encryption_path'))
      ->save();
  }
